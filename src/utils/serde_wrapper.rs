...
pub mod serializable_bigint {
    ...
    /// Serialize a BigInt to a JSON object with radix and value
    pub fn serialize<S>(_value: &Integer, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let format = BigIntFormat {
            radix: 16, // Default to base 16
            value: _value.to_string_base(16),
        };
...
pub mod serializable_vec_vec_bigint {
    ...
    pub fn serialize<S>(_value: &Vec<Vec<Integer>>, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
...
pub mod serializable_array_bigint {
    ...
    pub fn deserialize<'de, D, const N: usize>(deserializer: D) -> Result<[Integer; N], D::Error>
    where
        D: Deserializer<'de>,
    {
