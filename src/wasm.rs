...
use crate::{
    DecryptionKey, EncryptionKey, AnyEncryptionKeyExt,
    precomputed_table::PrecomputeTable,
};
...
impl PaillierWasm {
    ...
    pub fn encrypt(&mut self, plaintext: &str) -> Result<String, JsError> {
        let plaintext = BigInt::from_str_radix(plaintext, 10).map_err(|_| JsError::new("Invalid plaintext format"))?;
    ...
    pub fn encrypt_with_precomputed(&mut self, plaintext: i64) -> Result<String, JsError> {
        if let Some(precomputed) = &self.precomputed_table {
            ...
            let ciphertext = self.ek.encrypt_with_precompute_table(&mut self.rng, precomputed, &plaintext)
